    ...
    static class Lightmapping
    {
        ...
        public static void SetLightmapStaticFlagEnabled(ProBuilderMesh pb, bool isEnabled)
        {
            ...
            if (ent != null && ent.entityType == EntityType.Detail)
            {
                ...
                if (isEnabled != (flags & StaticEditorFlags.ContributeGI) > 0)
                {
        ...
        static UndoPropertyModification[] PostprocessModifications(UndoPropertyModification[] modifications)
        {
            ...
            foreach (var modification in modifications)
            {
                if (modification.currentValue.propertyPath.Equals(k_StaticEditorFlagsProperty))
                {
                    ...
                    var lightmapStatic = (staticFlags & (uint) StaticEditorFlags.ContributeGI) != 0;
        ...
        static void OnLightmappingCompleted()
        {
            ...
            var missingUv2 = Object.FindObjectsOfType<ProBuilderMesh>().Where(x => !x.HasArrays(MeshArrays.Lightmap) && x.gameObject.HasStaticFlag(StaticEditorFlags.ContributeGI));
        ...
        {
            int count = 0;
            float total = selection.Count(x => x.gameObject.HasStaticFlag(StaticEditorFlags.ContributeGI) && !x.HasArrays(MeshArrays.Lightmap));

            foreach (var mesh in selection)
            {
                if (!mesh.gameObject.HasStaticFlag(StaticEditorFlags.ContributeGI) || mesh.HasArrays(MeshArrays.Texture1))
                    continue;
